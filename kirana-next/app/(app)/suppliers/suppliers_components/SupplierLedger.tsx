...
import { useGetSupplier, useGetSettings } from "@/lib/api";
...
export function SupplierLedger({ supplierId }: SupplierLedgerProps) {
  const { data: detail, isLoading } = useGetSupplier(supplierId);
  ...
  const printThermalBill = () => {
    ...
    win.document.write(`
      ...
      <style>
        ...
        <hr/>
        <p><strong>Supplier:</strong> ${detail.name}</p>
  ...
  return (
    <div className="flex flex-col h-full p-6 overflow-hidden">
      ...
      <div className="flex justify-center mb-6">
        <div className="inline-flex flex-wrap gap-3 justify-center bg-[var(--supplier-btn-panel-bg)] border rounded-xl p-2 shadow-sm">
          <Button
            size="sm"
      ...
      {/* Ledger table */}
      <div className="flex-1 border rounded-xl bg-[var(--supplier-ledger-bg)] flex flex-col overflow-hidden">
        {/* Desktop header */}
